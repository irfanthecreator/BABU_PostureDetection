...
# <<< NEW: simple threaded beep so it doesn't freeze video
def beep_alert():
    def _beep():
...
def main():
    ...
    BAD_THRESHOLD = 1.0      # <<< NEW: seconds before we beep
    bad_beeped = False       # <<< NEW: to avoid nonstop beeping
        ...
        while True:
            ...
            if results.pose_landmarks and (now - last_update_time) > 0.3:
                ...
                if last_label == "bad":
                    ...
                    # <<< NEW: trigger beep once when threshold is crossed
                    ...
                    bad_start_time = now
                    bad_duration = 0.0
                    bad_beeped = False   # <<< NEW: reset when posture becomes good
